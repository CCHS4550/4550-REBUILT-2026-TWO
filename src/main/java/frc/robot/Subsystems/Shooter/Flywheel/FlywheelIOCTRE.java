...
public class FlywheelIOCTRE implements FlywheelIO{
    ...
    public FlywheelIOCTRE (BruinRobotConfig config){
        flywheelMotor1 = new TalonFX(config.FLYWHEEL_MOTOR_1.getDeviceNumber(), config.FLYWHEEL_MOTOR_1.getBus());
        flywheelMotor2 = new TalonFX(config.FLYWHEEL_MOTOR_2.getDeviceNumber(), config.FLYWHEEL_MOTOR_2.getBus());
        flywheelMotor3 = new TalonFX(config.FLYWHEEL_MOTOR_3.getDeviceNumber(), config.FLYWHEEL_MOTOR_3.getBus());

        Follower follower = new Follower(config.FLYWHEEL_MOTOR_1.getDeviceNumber(), MotorAlignmentValue.Aligned);
    ...
    public void updateInputs (FlywheelIOInputs inputs){
        ...
        inputs.flywheelVelocityRadPerSec = Units.rotationsToRadians(flywheelVelocityRotationsPerSec.getValueAsDouble());
        inputs.flywheelAccelRadPerSecPerSec = Units.rotationsToRadians(flywheelAccelerationRotationsPerSecSquared.getValueAsDouble());
    }
